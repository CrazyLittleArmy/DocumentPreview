...
nohup /app/software/jdk1.8.0_181/bin/java -Dfile.encoding=UTF-8 -Dsun.java2d.cmm=sun.java2d.cmm.kcms.KcmsServiceProvider -Dspring.config.location=../conf/application.properties -jar kkFileView-2.1.2.jar > ../log/kkFileView.log 2>&1 &
pid=`ps -ef | grep 'kkFileView-2.1.2.jar' | grep -v grep | awk '{print $2}'`
if [ -z "${pid}" ]; then
   echo "kkFileView-2.1.2.jar is not running"
else
    echo "kkFileView-2.1.2.jar is   running  lalalalla"
fi
